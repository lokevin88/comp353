<div id="newsfeed">
    ...
    <?php
        ...
        if(isset($_POST["submitGroupReplyPost-{$row['gPostsID']}"])) {
            ...
            $path = "<script>window.location = 'https://rrc353.encs.concordia.ca/comp353/src/pages/group-details-page.php?groupID=$groupID';</script>";
            echo $path;
            exit;
        }
    ?>
    <div class="card">
        ...
        <div class="card-footer">
            <div id="reply-<?php echo $row['gPostsID']; ?>" class="text-reply-color card-body-text" onclick="displayReplies(event);">reply (<?php echo $count_all_replies_on_member_posting ?>)</div>
                <?php
                    include $_SERVER['DOCUMENT_ROOT'] . 'https://rrc353.encs.concordia.ca/comp353/src/components/group-newsfeed-reply-component.php';
                ?>
        </div>
    </div>
    <?php endforeach; ?>
